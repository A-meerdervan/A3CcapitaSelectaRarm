...
class Worker():
    def __init__(self,name,s_size,a_size,trainer,model_path,tfSummary_path,global_episodes,global_rewardEndEpisode):
        ...
        self.termReward = 0.0 # used for the tf_summary. The last reward of the ep
    ...
    def work(self,max_episode_length,gamma,global_AC,sess,coord,saver):
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                while not done:
# This was done for pong S6, but maybe better to just only use the render way from main from arnold
...
#                        self.env.render()
                ...
                if episode_count % self.tfSummaryEpInterval == 0 and episode_count != 0: #TODO change to 5
                    ...
                    summary.value.add(tag='Terminal Reward', simple_value=float(self.termReward*200.0))
