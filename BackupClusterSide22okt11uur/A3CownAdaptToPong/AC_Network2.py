...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            hidden3 = slim.fully_connected(hidden2,64,activation_fn=tf.nn.relu)
            hidden4 = slim.fully_connected(hidden3,64,activation_fn=tf.nn.relu)
            hidden5 = slim.fully_connected(hidden4,64,activation_fn=tf.nn.relu)
