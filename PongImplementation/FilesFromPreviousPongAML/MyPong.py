...
if (gc.RENDER_SCREEN): screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
...
#            self.createWindow()
    ...
    def PlayNextMoveA3C(self, action, oppAction):
        self.gameTime += 1
        # This sets the render speed/ framerate! , Calculate DeltaFrameTime
        if gc.RENDER_SCREEN and gc.SHOW_NORMAL_SPEED:
            self.clock.tick(gc.FPS)
        # Use a constant to make slow motion viewing have the same game dynamics
        DeltaFrameTime = DFT
        #update our paddle
    ...
    def renderCurrentState(self):
            ...
            screen.blit(MovingAvrScoreDisplay,(50.,20.))
            ...
            screen.blit(ScoresDisplay,(50.,40.))
            TimeDisplay = self.font.render("Time: "+ str(self.GTimeDisplay), True,(255,255,255))
            screen.blit(TimeDisplay,(50.,40.))
            EpsilonDisplay = self.font.render("Ep: "+ str("{0:.4f}".format(self.GEpsilonDisplay)), True,(255,255,255))
            screen.blit(EpsilonDisplay,(50.,60.))
    ...
    def ReturnCurrentState(self):
        # Simply return state
        return [self.paddle1YPos, self.paddle2YPos, self.ballXPos, self.ballYPos, self.ballXDirection, self.ballYDirection]
    
    def returnCurrentStateOpp(self):
        return [self.paddle2YPos, self.paddle1YPos, self.ballXPos, self.ballYPos, self.ballXDirection, self.ballYDirection]
    ...
    def hasQuit(self):
        if not gc.RENDER_SCREEN: return False
